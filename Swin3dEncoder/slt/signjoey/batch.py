...
class Batch:
    ...
    def __init__(
        ...
        #print("inside batch self.sgn before", self.sgn.shape)
        ...
        if random_frame_masking_ratio and is_train:
            ...
            self.sgn = tmp_sgn
        #print("inside batch init sgn after", self.sgn.shape)
        self.sgn_dim = sgn_dim
        #print("inside batch init sgn dim", self.sgn_dim)
        self.sgn_mask = (self.sgn != torch.zeros(sgn_dim))[..., 0].unsqueeze(1)

        #print("inside batch init sgn mask", self.sgn_mask.shape)
        #print("mask with 0 or 1", self.sgn_mask[0][0])
